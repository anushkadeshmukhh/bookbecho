...
import mysql.connector #--------------neeche connctor
from django.urls import reverse
from django.shortcuts import redirect
users=""
try:
    mydb = mysql.connector.connect( #storing the connection
        host="localhost",
        user="root",
        password="Anushka",
        database="bokio"
...
def mysql():
    ...
    mycursor = mydb.cursor() #create mydb cha cursor 
...
def check_user(request): #check if valid
    template = loader.get_template('main.html')
    a = request.POST['username']
    get_user(a)
    e = request.POST['psw']
    aa=tp(a,e)  #call tp neeche
    #print(aa)
    fe=mydb.cursor()
    fe1=mydb1.cursor()
...
def add_user(request): #request the fields
    template = loader.get_template('main.html')
    a = request.POST['username']
    get_user(a)
    b = request.POST['name']
    ...
    val=(b,f,a,c,d,e)
    insert_user.execute(sql,val) #insert,what to pass in %s
    mydb.commit() #commit changes in database or wont go
    fe1=mydb1.cursor() #fe1 cursor by mydb1 object
    fe1.execute("select * from book")
...
def tp(a,e): #from above , we have manually used select, fetchall->all values for parameter passsed, execute->select statement
    tp=mydb.cursor()
    tp1="SELECT username FROM users WHERE username = %s"
    us_nm=(a,)
    tp.execute(tp1,us_nm) #executing the actual query of mysql select
    myresult = tp.fetchall()
    #print(myresult)
    if len(myresult)==0: #if no user in usr table, error,prompt
        return 0
    else:
        ...
        myresult = tp.fetchall()
        #print(myresult,e)
        if myresult[0][0]==e: #check for pwd
            return 1
        else:
            return 0
